...
#define MAX_TXNS ( 1 << 18 )
    ...
    for( ulong mblk = 0; mblk < mcount; ++mblk ) {
      ...
      for( ulong txn_idx = 0; txn_idx < hdr->txn_cnt; txn_idx++ ) {
        ...
        for( ulong j = 0; j < txn->signature_cnt; j++ ) {
          ...
          memcpy( &sig, sigs + j, sizeof(fd_blockstore_txn_key_t) );
  ...
  block->micro_gaddr = fd_wksp_gaddr_fast( fd_blockstore_wksp( blockstore ), micros_laddr );
  block->micro_cnt   = micros_cnt;
  ...
  block->txn_gaddr = fd_wksp_gaddr_fast( fd_blockstore_wksp( blockstore ), txns_laddr );
  block->txn_cnt   = txns_cnt;
}
...
fd_blockstore_slot_remove( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  if( FD_UNLIKELY( block_map_entry->consumed_idx != FD_SHRED_IDX_NULL &&
  ...
  fd_block_txn_ref_t * txns = fd_wksp_laddr_fast( wksp, block->txn_gaddr );
  for( ulong j = 0; j < block->txn_cnt; ++j ) {
    fd_blockstore_txn_key_t sig;
    memcpy( &sig, data + txns[j].id_off, sizeof(sig) );
  ...
  if( block->micro_gaddr ) fd_alloc_free( alloc, fd_wksp_laddr_fast( wksp, block->micro_gaddr ) );
  if( block->txn_gaddr ) fd_alloc_free( alloc, txns );
...
deshred( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  block->shred_gaddr = fd_wksp_gaddr_fast( wksp, shreds_laddr );
  block->shred_cnt   = shred_cnt;
  ...
  case FD_SHRED_ESLOT:
    ...
    fd_block_micro_t *    micros     = fd_wksp_laddr_fast( wksp, block->micro_gaddr );
    uchar *               data       = fd_wksp_laddr_fast( wksp, block->data_gaddr );
    fd_microblock_hdr_t * last_micro = (fd_microblock_hdr_t *)( data +
                                                                micros[block->micro_cnt - 1].off );
...
fd_blockstore_shred_insert( fd_blockstore_t * blockstore, fd_shred_t const * shred ) {
  ...
  if( FD_UNLIKELY( !block_map_entry ) ) {
    ...
    block_map_entry->consumed_idx   = FD_SHRED_IDX_NULL;
    block_map_entry->received_idx   = shred->idx + 1;
    block_map_entry->complete_idx   = FD_SHRED_IDX_NULL;
  ...
  while( fd_blockstore_shred_query( blockstore, shred->slot, (uint)( block_map_entry->consumed_idx + 1U ) ) ) {
    block_map_entry->consumed_idx++;
  }
  ...
  if( FD_LIKELY( block_map_entry->consumed_idx == FD_SHRED_IDX_NULL ||
                 block_map_entry->consumed_idx != block_map_entry->complete_idx ) ) {
    return FD_BLOCKSTORE_OK;
  }
...
fd_blockstore_shred_query_volatile( fd_blockstore_t * blockstore,
...
#if FD_BLOCKSTORE_USE_HANDHOLDING
    if( FD_UNLIKELY( sz > FD_SHRED_MAX_SZ ) ) return -1;
#endif
  ...
  fd_block_shred_t * shreds = fd_wksp_laddr_fast( wksp, blk->shred_gaddr );
  ulong sz = fd_shred_payload_sz( &shreds[shred_idx].hdr );
  memcpy( buf, &shreds[shred_idx].hdr, FD_SHRED_DATA_HEADER_SZ );
...
int
fd_blockstore_child_slots_query( fd_blockstore_t * blockstore,
                                 ulong             slot,
                                 ulong **          slots_out,
                                 ulong *           slot_cnt_out ) {
  fd_block_map_t * query = fd_blockstore_block_map_query( blockstore, slot );
...
fd_blockstore_txn_query( fd_blockstore_t * blockstore, uchar const sig[FD_ED25519_SIG_SZ] ) {
  ...
  memcpy( &key, sig, sizeof(fd_blockstore_txn_key_t) );
...
fd_blockstore_txn_query_volatile( fd_blockstore_t * blockstore, uchar const sig[FD_ED25519_SIG_SZ], fd_blockstore_txn_map_t * txn_out, long * blk_ts, uchar * blk_flags, uchar txn_data_out[FD_TXN_MTU] ) {
  ...
  for(;;) {
    ...
    memcpy( txn_out, txn_map_entry, sizeof(fd_blockstore_txn_map_t) );
...
fd_blockstore_log_mem_usage( fd_blockstore_t * blockstore ) {
  ...
  while( !fd_blockstore_slot_deque_empty( q ) ) {
    ...
    if( block->data_gaddr ) {
      ...
      txn_tot += block->txn_cnt;
      txn_max = fd_ulong_max( txn_max, block->txn_cnt );
    }
...
fd_blockstore_init( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  block_map_entry->parent_slot     = FD_SLOT_NULL;
  for( ulong i = 0; i < FD_BLOCKSTORE_CHILD_SLOT_MAX; i++ ) {
    block_map_entry->child_slots[i] = FD_SLOT_NULL;
  }
  block_map_entry->child_slot_cnt  = 0;
  ...
  block_map_entry->block_hash      = ( fd_hash_t ){ 0 };
  block_map_entry->bank_hash       = ( fd_hash_t ){ 0 };
  block_map_entry->merkle_hash     = ( fd_hash_t ){ 0 };
  ...
  /* This creates an empty allocation for a block for slot, to "facade"
     that we have this particular block (even though we don't).  This is
     useful to avoid special-casing various blockstore APIs. */
