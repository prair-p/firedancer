...
/* fd_vm_check_align_enabled returns 1 if the vm should check alignment
   when doing memory translation. */
FD_FN_PURE static inline int
fd_vm_check_align_enabled( fd_vm_t const * vm ) {
   return !vm->is_deprecated;
}

/* fd_vm_check_size_enabled returns 1 if the vm should check size
   when doing memory translation. */
FD_FN_PURE static inline int
