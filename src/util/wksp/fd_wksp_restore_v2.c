...
/* fd_wksp_private_restore_v2_task restores a cgroup's allocation into
...
static int
fd_wksp_private_restore_v2_task( fd_wksp_t *                      wksp,
                                 fd_restore_t *                   restore,
                                 fd_wksp_checkpt_v2_hdr_t const * hdr,
                                 ulong                            frame_off_lo,
                                 ulong                            frame_off_hi,
...
/* fd_wksp_private_restore_v2_parallel replaces all the allocations in a
...
static int
fd_wksp_private_restore_v2_parallel( fd_wksp_t *    wksp,
                                     fd_restore_t * restore,
                                     ulong          restore_sz,
                                     uint           new_seed ) {
  ulong frame_off;
...
//RESTORE_SEEK( frame_off_hdr );
  ...
  while( frame_off_appendix ) {
    ...
    /* At this point, we know how to do an embarassingly parallel restore
       directly into the wksp.  E.g. in a tpool, every worker thread would
       have its own restore, run the task below for each cgroup assigned
       to it with reduction of the local error and dirty returns to this
       thread.  On completion of all tasks, this thread would branch to
       fail if an err was deteceted in any of the threads. */

    FD_LOG_INFO(( "Restoring volume cgroups" ));

    for( ulong cgroup_idx=0UL; cgroup_idx<cgroup_cnt; cgroup_idx++ ) {
      int dirty_thread;
      int err_thread = fd_wksp_private_restore_v2_task( wksp, restore, hdr,
                                                        cgroup_frame_off[ cgroup_idx ], cgroup_frame_off[ cgroup_idx+1UL ],
                                                        cgroup_pinfo_lo [ cgroup_idx ], cgroup_pinfo_lo [ cgroup_idx+1UL ],
                                                        &dirty_thread ); /* logs details */
      dirty |= dirty_thread;
      if( FD_UNLIKELY( err_thread ) ) goto fail;
    }
...
/* fd_wksp_private_restore_v2_stream is identical to
   fd_wksp_private_restore_v2_parallel (above) but used when restore is
   not seekable (e.g. the restore is from a pipe or socket but this will
   work fine if used on mmio restores too).  Restore must be compactly
   stored.  Exact same behaviors. */
...
int
fd_wksp_private_restore_v2( fd_wksp_t *  wksp,
                            char const * path,
                            uint         new_seed ) {
  ...
  if( FD_LIKELY( !err ) ) {
    FD_LOG_INFO(( "using mmio" ));
    restore = fd_restore_init_mmio( _restore, mmio, mmio_sz ); /* logs details */
    /* FIXME: considering trimming off prefix and suffix to mmio region
       here (scan for MAGIC and ~MAGIC) */
  } else {
    FD_LOG_INFO(( "\"%s\" does not appear to support mmio (%i-%s); using streaming", path, err, fd_io_strerror( err ) ));
    restore = fd_restore_init_stream( _restore, fd, rbuf, FD_RESTORE_RBUF_MIN ); /* logs details */
  }
  if( FD_UNLIKELY( !restore ) ) goto fail;

  FD_LOG_INFO(( "Restoring checkpt" ));

  /* If the restore is not seekable (e.g. from a pipe or a socket),
     restore via the streaming algo */

  ulong restore_sz = fd_restore_sz( restore );
  if( FD_LIKELY( restore_sz!=ULONG_MAX ) ) {
    FD_LOG_INFO(( "restore appears seekable; trying a parallel restore" ));
    err = fd_wksp_private_restore_v2_parallel( wksp, restore, restore_sz, new_seed ); /* logs details */
  } else {
    FD_LOG_INFO(( "restore does not appear seekable (%i-%s); trying a streaming restore", err, fd_checkpt_strerror( err ) ));
