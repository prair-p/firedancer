...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->topo = topo;
  ctx->gui  = fd_gui_join( fd_gui_new( _gui, gui_hcache, ctx->version_string, tile->http.cluster, ctx->identity_key, tile->http.is_voting, ctx->topo ) );
  FD_TEST( ctx->gui );
  ...
  FD_LOG_WARNING(( "GUI server listening on port %u", tile->http.gui_listen_port ));
  FD_LOG_WARNING(( "Prometheus metrics endpoint listening on port %u", tile->http.prometheus_listen_port ));
}
