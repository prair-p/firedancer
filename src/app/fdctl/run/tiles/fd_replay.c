...
fd_exec_packed_txns_task( void *tpool,
  ...
  // FD_LOG_WARNING(("MBLK5: %lu %lu", curr_slot, seq));
  ...
  // FD_LOG_WARNING(("MBLK6: %lu %lu", curr_slot, seq));
}
  ...
  if( in_idx == STORE_IN_IDX ) {
    ...
    */
    ...
    if( fd_disco_poh_sig_pkt_type( sig )==POH_PKT_TYPE_MICROBLOCK ) {
      // FD_LOG_WARNING(("MBLK2: %lu %lu", ctx->curr_slot, seq));
...
static void
publish_notifications( fd_replay_tile_ctx_t * ctx, 
                       fd_fork_t *            fork,
...
after_frag( void *             _ctx,
  ...
  if( fork == NULL ) {
    fork = prepare_new_block_execution( ctx, mux, curr_slot, flags );
  }
  ...
  long execute_time_ns = -fd_log_wallclock();
    ...
    if( flags & REPLAY_FLAG_FINISHED_BLOCK ) {
      ...
      publish_notifications( ctx, fork, block_map_entry, curr_slot );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  if( tile->replay.cluster_version ) {
    ctx->epoch_ctx->epoch_bank.cluster_version = tile->replay.cluster_version;
